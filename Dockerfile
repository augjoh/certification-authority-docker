...
RUN chmod 755 /usr/bin/entrypoint.sh \
 ...
 && ln -sf $(pwd)/Sub/https.crt.pem /etc/ssl/apache2/server.pem \
 && ln -sf $(pwd)/Sub/https.priv.key.pem /etc/ssl/apache2/server.key \
 && ln -sf $(pwd)/Sub/ca.crt.pem /etc/ssl/apache2/server-ca.pem \
