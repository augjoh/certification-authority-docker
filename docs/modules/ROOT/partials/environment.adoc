...
`APACHE_PID_FILE`::
  Process IDentifier (PID) file to be deleted, when container starts up, by
  default `/run/httpd/httpd.pid`.
`APACHE_ENABLE_RSA`::
...
`APACHE_ENABLE_EC`::
  ...
  certificate generation, too.
`APACHE_ENABLE_EDDSA`::
  enable EdDSA TLS certificate usage, when specified as `true`. By default
  EdDSA certificate isn't used. See `HTTPS_CERTIFICATE_ALGORITHM` how to
  enable automatic certificate generation and renewals, too.
